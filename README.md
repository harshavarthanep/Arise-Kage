...
workout logging with rest timers and PRs, custom routines, water tracking,
cross-training activity log, progress photos saved to IndexedDB, XP
reversal on delete, Ask Kage Q&A, quests/achievements, theme customization,
JSON backup/restore, and PWA install/offline behavior) — verified with
automated browser tests across mobile, tablet, and desktop viewports on
both the single-file and modular builds independently.
...
rest timer with an audible chime (toggleable), automatic PR detection,
"Repeat Last Workout" one-tap restart, water tracking with quick presets
and a 7-day history chart, body metrics (weight/measurements, deletable)
with a trend sparkline, sleep logging, and a lightweight cross-training
**Activity Log** for anything that isn't barbell work (running, cycling,
football, cricket, swimming, basketball, yoga, other) — duration,
intensity, optional distance, feeding the same streak/XP/quest system.
...
modular/
  ...
  js/
    ...
    activity.js           cross-training log (running/cycling/sport/etc.)
    ...
    bootstrap.js          theming (incl. Auto/system mode), PWA setup, easter eggs, app boot sequence
```

---

Built 2026-08-12, revised 2026-08-13. No subscriptions, no ads, no locked features — and none
